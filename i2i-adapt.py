...
def train(source_encoder, source_decoder, target_encoder, target_decoder,
    ...
    for i in range(epochs):
        ...
        for batch_idx, ((data_t, _), (data_s, target_s)) in enumerate(data_zip):
            ...
            c_loss = F.nll_loss(output, target_s)
            ...
            id_loss = nn.MSELoss()(data_s, source_decoding) + nn.MSELoss()(data_t, target_decoding)
            ...
            z_loss = F.nll_loss(z_out.float().requires_grad_(), z_label)
            ...
            tr_loss = F.nll_loss(tr_out.float().requires_grad_(), tr_label)
            ...
            cyc_loss = nn.MSELoss()(data_s, source_cycle) + nn.MSELoss()(data_t, target_cycle)

            # 6 Source to target classification loss
            output_trc = class_classifier(source_target_encoding)
            trc_loss = F.nll_loss(output_trc, target_s)
